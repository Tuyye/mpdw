...
```{r}
index <- seq(1:115)
bc = boxcox(dat.latih.ts~index, lambda = seq(-0.6,3.5,by=0.1))
...
identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(2,0,2), dan ARIMA (3,0,2)

sehingga model tentafifnya ialah ARIMA(0,0,1) ARIMA(2,0,0), ARIMA(2,0,2), ARIMA(3,0,2)
...
```{r}
model4=Arima(dat.latih.ts, order=c(3,0,2),method="ML")
summary(model4) #AIC=930.95
...
```{r}
model5=Arima(dat.latih.ts, order=c(3,0,3),method="ML")
summary(model5) #AIC=416.65 
lmtest::coeftest(model5) #Tidak signifikan
```

berdasarkan hasil overfitting yg terbaik adalah model ARIMA(3,0,3)
